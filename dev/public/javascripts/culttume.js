      ...
      finish: function() {
        ...
        if (data.services.length > 0 && data.profile !== 'Escoge una opción' && data.askAbout !== 'Escoge una opción' && data.name !== "" && data.name.length >= 5 && data.city !== "" && data.city.length >= 3 && data.age !== "" && data.age.length > 1 && data.age.length < 3 && data.genre !== 'Selecciona tu Genero') {
          ...
          socket.on('congratulationDetail', function(congrat) {
            $('#congratulationDetail').append('<div class="ten centered columns">' + '<span>' + congrat.name + '</span><span class="subheader"> Bienvenido a culttu.me</span>' + '</div>' + '<div class="twelve columns">' + '<hr class="lineFooterCulttume"></div>' + '<p class="subheader"> Recuerda que te enviaremos información y novedades' + ' al correo electrónico:</p>' + '<div class="alert-box success">' + congrat.email + '</div><br />' + '<p class="subheader">Adicional ganas <span class="radius label"> 200 Puntos</span> que podras canjear por premios' + 'y todas las cosas que más te gustan de tu artista favorito</p>' + '<div class="three columns centered">' + '<img src="./img/modal/cup.png" /></div>');
