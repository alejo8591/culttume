(function() {
  ...
  jQuery.validator.addMethod('verificationRegisterCode', function(value, element) {
    this.resultado = false;
    socket.emit('validationCodeRequest', {
      code: $('#registerCode').val(),
      email: $('#email').val()
    });
    socket.on('validationCodeResponse', function(data) {
      console.log(data.status !== 4 ? false : true);
      return this.resultado = data.status !== 4 ? false : true;
    });
